...
buildx-build-image-deviceshifu-http-http:
	cd shifu && docker buildx build --platform=linux/$(go env GOARCH) -f dockerfiles/Dockerfile.deviceshifuHTTP \
		-t edgehub/deviceshifu-http-http:nightly . --load && cd ..

build-shifu-image:
	cd shifu && docker buildx build --platform=linux/$(go env GOARCH) -f pkg/k8s/crd/Dockerfile -t edgehub/shifu-controller:nightly . --load && cd ..
